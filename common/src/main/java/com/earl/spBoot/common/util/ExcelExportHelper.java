...
public class ExcelExportHelper {
     ...
     * 默认构造函数
     */
    public ExcelExportHelper(int defaultWidth){
    ...
    private void setExcelContentData(HSSFWorkbook book,HSSFSheet sheet,String[] header,List<Object> excelList) {
        ...
        // 创建头部
        HSSFRow row = createHeader(sheet, headerStyle, header,0);
    ...
    public HSSFWorkbook setExcelContentData(HSSFWorkbook book, HSSFSheet sheet, String[] header, String[] properties,
                                     List<?> excelList) {
        //设置列头样式(居中、变粗、蓝色)
        HSSFCellStyle headerStyle = book.createCellStyle();
        setHeaderStyle(headerStyle, book);
        ...
        while (it.hasNext()) {
            ...
            for(int i = 0 ; i < header.length ; i++){
                ...
                try {
                    ...
                    System.out.println("————创建Excel数据列表时出错。没有找到属性方法："+properties[i]);
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                    System.out.println("——————————————————创建Excel数据列表时出错。method:setDataRow,message："+e.getMessage());
                } catch (IllegalArgumentException e) {
     ...
     * @version 1.0
     */
    public  String getSheetTitle(String sheetTitle) {
        String title = null;
     ...
     * @version 1.0
     */
    public void setHeaderStyle(HSSFCellStyle headerStyle,HSSFWorkbook book) {
     ...
     * @version 1.0
     */
    public void setCellStyle(HSSFCellStyle cellStyle, HSSFWorkbook book) {
     ...
     * @version 1.0
     */
    public HSSFRow createHeader(HSSFSheet sheet,HSSFCellStyle headerStyle,
                                 String[] header,int row) {
     ...
     * @version 1.0
     */
    public void setCellData(HSSFRow row, int index ,int i ,Object value,HSSFCell cell,HSSFSheet sheet,HSSFPatriarch patriarch,HSSFWorkbook book) {
    ...
    private void zipExcelAndSave(List<HSSFWorkbook> books,String filePath,String zipName,String excelName){
        ...
        try {
            output = new FileOutputStream(filePath + "\\" + zipName + ".zip");
    ...
    /**
     * 检查字符串是否url 图片地址
     * @param value 若是则返回一个object类型的图片byte[]数组，若不是，原值返回
     * @return
     */
    private Object checkUrlImage(Object value){
        String regex =  "http?://.+\\.(jpg|gif|png)";
        //是否url地址,检查是否图片地址
        if(match(regex, value.toString())){
            try {
                ...
                System.out.println("---导出excel.获取图片错误:"+value.toString()+"----");
            }
        }
        return value;
    }
    ...
    /**
     * 在某个sheet追加数据(针对追加统计单行)
     * @param wb HSSFWorkbook
     * @param head 栏目标题
     * @param property 栏目内容
     * @param sheet 要追加到的哪个HSSFSheet
     * @param beginRow 追加行数下标，从哪一行开始追加（第一行为0）
     * @return HSSFWorkbook
     */
    public HSSFWorkbook appendRowData(HSSFWorkbook wb,
                                         String head [],String property[],  HSSFSheet sheet,int beginRow){
        if (beginRow <= -1){
            beginRow = 0;
        }
        //设置列头样式(居中、变粗、蓝色)
        HSSFCellStyle headerStyle = wb.createCellStyle();
        setHeaderStyle(headerStyle, wb);

        // 设置单元格样式
        HSSFCellStyle cellStyle = wb.createCellStyle();
        setCellStyle(cellStyle, wb);
        // 创建头部
        HSSFRow row = createHeader(sheet, headerStyle, head,beginRow);
        // 画图的顶级管理器，一个sheet只能获取一个（一定要注意这点）
        HSSFPatriarch patriarch = sheet.createDrawingPatriarch();
         //标题行的下一行（行数据位置）
        int index = beginRow + 1;
        ////创建数据行
        row = sheet.createRow(index);
        maxWidth = new int[head.length];   //初始化单元格宽度
        HSSFCell cell = null;
        for (int i = 0;i < property.length;i++) {
            cell = row.createCell(i);
            cell.setCellStyle(cellStyle);
            setCellData(row,index, i, property[i], cell, sheet, patriarch, wb);
        }
        return wb;
    }
