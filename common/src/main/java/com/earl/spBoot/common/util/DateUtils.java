...
public class DateUtils {
    ...
    public static Date string2Date(String value){
        SimpleDateFormat df = new SimpleDateFormat(DateFormatUtils.DATE_FORMAT2);
        Date date = null;
        try {
            date = df.parse(value);
        } catch (ParseException e) {
     ...
     * 字符串日期转字符串日期
     * @param strDate 字符串日期
     * @param format  原来的字符窜日期格式.比如：yyyy-mm-dd
     * @param resultFormat 返回的字符窜日期格式,比如：yyyy-mm-dd
     * @return 字符串日期
     */
    public static String string2StringDate(String strDate,String format,String resultFormat)  {
        Date date = string2Date(strDate, resultFormat);
        return date2String(date, resultFormat);
    }
