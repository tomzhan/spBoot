    ...
    _ajax: function (_type, _url, _params, _callback, isJson, async, isLoading) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (!succ) {
                    Alert.error('错误',data.error);
                }
                _callback(data, succ);
            },
            error: function () {
                Alert.error('错误','发起请求时出现错误!');
            },
            complete: function () {
                //关闭loading层
                if (isLoading != undefined && isLoading == true) {
...
var Alert = {
     ...
     * @param callback 回调函数
     * @param time 根据多少秒关闭，不填写默认为2秒.单位1000为1秒
     * @param isShock 是否开启震动的弹框，true表示开启，默认不开启
     */
    ...
    error: function (title, text, callback) {
        layer.alert( text, {icon:2,title:title,anim:6},function (index) {
            layer.close(index);
        },callback);
    },
